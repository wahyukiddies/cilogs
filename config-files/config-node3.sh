...
create_cron_job() {
    ...
    # Run the backup_job.sh script at 00:00 every ${BACKUP_IN_DAYS}
    # YOU CAN CHANGE THE CRON JOB SCHEDULE ACCORDING TO YOUR NEED
    crontab << EOF
0 0 */${BACKUP_IN_DAYS} * * /root/backup-job.sh
EOF
